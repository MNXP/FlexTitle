...
public class ViewPagerTitle extends HorizontalScrollView {
    ...
    private int getTextViewMargins(String[] titles) {
        ...
        if (titleCenter){
            ...
            countLength = countLength + 2 * itemMargins + paint2.measureText(titles[0]);
            int screenWidth = getScreenWidth(getContext());

            if (countLength <= screenWidth) {
                allTextViewLength = screenWidth;
                return (screenWidth - (int) paint.measureText(titles[0]) * (titles.length - 1) - (int) paint2.measureText(titles[0])) / (titles.length * 2);
            } else {
            ...
            if (countLength <= screenWidth) {
                allTextViewLength = screenWidth;
                return (screenWidth - (int) paint.measureText(titles[0]) * (titles.length - 1) - (int) paint2.measureText(titles[0])) / (titles.length + 1);
            } else {
                allTextViewLength = (int) countLength;
                return (int) itemMargins;
            }
